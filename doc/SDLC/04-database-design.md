...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity                               | `users`, `operator_accounts`, `employee_accounts`, `platform_accounts`, `auth_sessions`, `mfa_credentials`, `mfa_backup_codes` | `iam/` (auth, user, session, role)                                 | 3 namespace identity (Passenger Email / Operator `{slug}/{username}` / Platform `platform/{username}`), Account-separate (ADR-017). RBAC 8-role = enum hardcoded v1 (chưa cần bảng `permissions`). `auth_sessions` giữ opaque refresh token 30d (rotation + family) |
