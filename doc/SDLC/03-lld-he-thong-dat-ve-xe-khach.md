...
| 4    | Phát hành Hybrid token: JWT RS256 access 15min (claims `sub`, `scope`, `role`, `operatorSlug`) + opaque 32-byte refresh 30d persist `auth_sessions`; cache session metadata Redis. Web=httpOnly cookie, Mobile=`flutter_secure_storage`. |
